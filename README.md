...
- 多窗口支持：父进程 PID 映射隔离，每个 CC 会话独立信号灯
...
hooks 驱动 (VSCode + Terminal 通用):
  CC hooks 事件 → main.py hook <status> <message>
    → 查找所属灯 (三级优先级):
      ...
      ② .map-{ppid} 父进程 PID 映射 (区分不同 CC 会话)
      ③ 心跳扫描 (跳过已被其他会话认领的灯)
    → 首次触发时懒创建新灯 + 写入 .map-{ppid} 绑定
    → 写入 status-{id}.json
...
每个 CC 会话的 hooks 进程父 PID 不同:
  Terminal CC → bash.exe → hooks 父 PID = bash PID
  VSCode CC   → VS Code ext host → hooks 父 PID = ext host PID
因此 .map-{ppid} 映射自然隔离，多窗口各自拥有独立信号灯。
```
